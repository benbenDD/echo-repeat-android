...
class TrackDao(private val db: AppDatabase) {
    ...
    private fun Cursor.toTrack() = TrackEntity(
        ...
        title=getString(getColumnIndexOrThrow("title")), subtitleUri=getStringOrNull("subtitleUri"), durationMs=getLong(getColumnIndexOrThrow("durationMs")),
    ...
    private fun TrackEntity.values(includeId: Boolean) = ContentValues().apply {
        if(includeId) put("id",id);put("audioUri",audioUri);put("fileName",fileName);put("title",title);put("subtitleUri",subtitleUri);put("durationMs",durationMs)
